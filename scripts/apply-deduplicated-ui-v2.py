...
if 'Open Bills or Transactions to confirm the missing evidence.' not in app:
    raise SystemExit('Overview attention guidance target missing')
...
MOBILE.write_text(mobile.replace(old_nav_css, new_nav_css, 1))
...
AUDIT.write_text(audit.replace(anchor, addition, 1))
...
release = '2026-08-30-deduplicated-ui-v1'
VERSION.write_text('{\n  "version": "' + release + '"\n}\n')
manifest = MANIFEST.read_text()
manifest, count = re.subn(r'("start_url"\s*:\s*"/penny-budget/\?v=)[^"]+("\s*,?)', rf'\g<1>{release}\2', manifest, count=1)
if count != 1:
    raise SystemExit('Manifest start_url version target missing')
MANIFEST.write_text(manifest)

print('Applied Penny deduplicated UI patch on latest main')
