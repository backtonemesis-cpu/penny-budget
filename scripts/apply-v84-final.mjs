...
function replaceOnce(text, before, after, label) {
  ...
  if (first < 0) throw new Error(`v84 missing anchor: ${label}`);
  if (text.indexOf(before, first + before.length) >= 0) throw new Error(`v84 ambiguous anchor: ${label}`);
...
`;
  ...
  const transactionsCallAfter = `          <Transactions
            ...
            canEdit={canEditMonth}`;
  app = replaceOnce(app, transactionsCallBefore, transactionsCallAfter, 'Transactions assignment options');
  ...
  const expenseCallBefore = `            <ExpenseRow
              key={transaction.id}
              ...
              accountMap={accountMap}
              canEdit={canEdit}`;
  const expenseCallAfter = `            <ExpenseRow
              key={transaction.id}
              transaction={transaction}
              categoryMap={categoryMap}
              peopleMap={peopleMap}
              ...
              canEdit={canEdit}`;
  app = replaceOnce(app, expenseCallBefore, expenseCallAfter, 'Expense row assignment options');

  app = replaceOnce(
    app,
    '              onEdit={onEditTransaction}\n              onDelete={onDeleteTransaction}',
    '              onEdit={onEditTransaction}\n              onAssign={onAssignTransaction}\n              onDelete={onDeleteTransaction}',
    'Expense row assignment handler',
  );
...
function setupReferenceLists(state, sourceMonthKey, targetMonthKey) {
  ...
  const targetPeople = state?.peopleByMonth && Object.hasOwn(state.peopleByMonth, targetMonthKey) ? getMonthPeople(state, targetMonthKey) : [];
  const targetAccounts = state?.accountsByMonth && Object.hasOwn(state.accountsByMonth, targetMonthKey) ? getMonthAccounts(state, targetMonthKey) : [];
  return {
...
function resolveSetupAccount(record, ownerId, accounts) {
  ...
  accounts.filter((account) => setupAccountOwnerCompatible(account, ownerId)).forEach((account) => {
    ...
    if (!candidate) return;
    if (evidence.some((label) => label === candidate || (label.length >= 4 && candidate.length >= 4 && (label.includes(candidate) || candidate.includes(label))))) matches.set(account.id, account);
  });
  if (matches.size === 1) return [...matches.values()][0];
  if (matches.size > 1) return null;
  if (ownerId && ownerId !== 'unassigned' && ownerId !== 'household') {
    const owned = accounts.filter((account) => account.ownerId === ownerId);
...
sourceAudit = sourceAudit.split('\n').map((line) => {
  if (line.includes('Income Detail must expose an actionable recipient assignment.')) return "  assert.match(files.app, /onAssignIncome\\(record, 'receivedBy', value\\)/, 'Income Detail must expose an actionable recipient assignment.');";
  if (line.includes('Income Detail must expose an actionable receiving-account assignment.')) return "  assert.match(files.app, /onAssignIncome\\(record, 'account', value\\)/, 'Income Detail must expose an actionable receiving-account assignment.');";
  return line;
}).join('\n');
await writeFile(sourceAuditPath, sourceAudit);

console.log('PENNY_V84 final inline assignment and rollover patch applied');
