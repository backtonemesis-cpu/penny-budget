...
replaceRequired('          amount: amountConfirmed ? amount : 0,', '          amount: amountConfirmed ? amountCheck.value : 0,', 'income safe amount value');
replaceRequired('        amount,\n        desc: description,', '        amount: amountCheck.value,\n        desc: description,', 'expense safe amount value');
...
// Inject a dedicated explanation next to the month setup controls rather than
// depending on older paragraph wording, which has changed across releases.
replaceRequired(
  '    <SimpleModal title={`Set up ${targetLabel}`} onClose={onClose} wide>',
  '    <SimpleModal title={`Set up ${targetLabel}`} onClose={onClose} wide>\n      <p className="section-note savings-copy-explanation">Savings snapshot is separate: if you switch it off, Penny does not copy the savings account definitions or their balances, so Savings for the new month starts empty.</p>',
