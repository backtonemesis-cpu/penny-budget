...
if (!app.includes(dedicatedExpenseRoute)) {
  ...
  if (!legacyRoute) {
    throw new Error('v35 could not find the Overview Expenses navigation route. Refusing to build with ambiguous Expenses navigation.');
  }
...
if (!app.includes("{view === 'Expenses' && (")) {
  ...
  if (!app.includes(incomeViewAnchor)) {
    throw new Error('v35 could not find the Income detail view anchor needed to install Expense Detail.');
  }
  app = app.replace(
    incomeViewAnchor,
...
if (!app.includes('function ExpenseDetail({')) {
  ...
  if (!app.includes(incomeDetailAnchor)) {
    throw new Error('v35 could not find IncomeDetail to install the matching ExpenseDetail component.');
  }
  app = app.replace(incomeDetailAnchor, `${expenseDetail}${incomeDetailAnchor}`);
}

if (!app.includes(dedicatedExpenseRoute)) {
  throw new Error('v35 failed: Overview Expenses is not routed to the dedicated Expense Detail view.');
}
if (legacyExpenseRoutes.some((candidate) => app.includes(candidate))) {
  throw new Error('v35 failed: a legacy Overview Expenses -> Transactions route still exists.');
}
if (!app.includes("{view === 'Expenses' && (") || !app.includes('function ExpenseDetail({')) {
  throw new Error('v35 failed: dedicated Expense Detail view/component is incomplete.');
}

await writeFile(appPath, app);
console.log('PENNY_V35_EXPENSE_DETAIL applied with verified dedicated routing');
