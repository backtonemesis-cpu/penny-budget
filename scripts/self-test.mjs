...
const state = {
  ...
  incomeByMonth: {
    '2026-06': [
      normaliseIncomeRecord({ id: 'j1', date: '2026-06-01', amount: 2500, description: 'June income', incomeType: 'Employment', receivedBy: 'p1', account: 'a1' }, '2026-06'),
    ],
    '2026-07': [
  ...
  txnsByMonth: {
    '2026-06': [
      normaliseTransaction({ id: 'je1', type: 'expense', date: '2026-06-01', amount: 500, desc: 'June cost', category: 'other', expenseClass: 'variable', paid: true, paidBy: 'p1', account: 'a1' }),
    ],
    '2026-07': [
...
assert.equal(mergedJune.isComplete, true);
assert.equal(mergedJune.startingSavings, 6500);
assert.equal(mergedJune.currentSavings, 8500);
assert.equal(mergedJune.income, 2500);
...
assert.equal(annual.income, 6500);
assert.equal(annual.expenses, 2500);
assert.equal(annual.savedThisMonth, 4000);
...
console.log('Penny historical reconciliation, month-specific savings, merge-import and finance tests passed');
