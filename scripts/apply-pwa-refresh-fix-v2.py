...
Path('public/version.json').write_text('{\n  "version": "2026-08-30-pwa-refresh-v2"\n}\n')
manifest = Path('public/manifest.webmanifest')
original_manifest = manifest.read_text()
updated_manifest = original_manifest.replace('/penny-budget/?v=2026-08-30-deduplicated-ui-v1', '/penny-budget/?v=2026-08-30-pwa-refresh-v2')
...
old_test = "node scripts/modal-layout-test.mjs && node scripts/source-audit.mjs"
new_test = "node scripts/modal-layout-test.mjs && node scripts/update-refresh-test.mjs && node scripts/source-audit.mjs"
if old_test not in pt:
    raise SystemExit('package test target not found')
package.write_text(pt.replace(old_test, new_test, 1))
print('Applied iPhone/PWA foreground update verification fix on current main')
