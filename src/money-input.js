...
export function validateMoneyInput(value, { allowZero = false } = {}) {
  ...
  try {
    const pence = (BigInt(whole) * 100n) + BigInt((decimals + '00').slice(0, 2));
    if (pence > MAX_SAFE_MONEY_PENCE) return { ok: false, code: 'unsafe' };
    if (!allowZero && pence === 0n) return { ok: false, code: 'zero' };
...
export function moneyValidationMessage(result, label = 'Amount') {
  switch (result?.code) {
    ...
    case 'unsafe': return `${label} is too large to store safely to the penny. Enter ${MAX_SAFE_MONEY_DISPLAY} or less.`;
